...
def validate_response(response, client_challenge: bytes):
    ...
    gc.collect()
    message = shared_secret + client_challenge + body
    signature_bytes = _b64decode(signature)
    if not verify_pkcs1v15_sha256(message, signature_bytes, ORIGIN_PUBLIC_KEY_N, ORIGIN_PUBLIC_KEY_E):
