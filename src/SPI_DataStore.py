...
memory_map = {
    ...
    "tournament": {"start": top_mem - 16 - (20 * 30) - (35 * 20) - (12 * 100), "size": 12, "count": 100},
    "individual": {"start": top_mem - 16 - (20 * 30) - (35 * 20) - (12 * 100) - (14*20*30), "size": 14, "count": 20, "sets": numberOfPlayers},  #count(scores) of 20, and 30 players
    "configuration": {"start": top_mem -16 -(20 * 30) - (35 * 20) - (12 * 100) - (14*20*30) - (96*1), "size": 96, "count":1 }, 
    "extras": {"start": top_mem -16 -(20 * 30) - (35 * 20) - (12 * 100) - (14*20*30) - (96*1) - (48*1), "size": 48, "count":1 }   
} 
...
def serialize(record, structure_name):
    if structure_name == "names":
        ...
        return struct.pack("<3s16s10sI", record["initials"].encode(), record["full_name"].encode(),record["date"].encode(), record["score"] )
    elif structure_name == "tournament":
        return struct.pack("<3sIBB", record["initials"].encode(), record["score"], record["game"], record["index"])
...
def deserialize(data, structure_name):
    if structure_name == "names":
        try:
            ...
            return {"initials": initials.decode().strip('\0'), "full_name": name.decode().strip('\0')}
        except:
            return {"intials": " ","full_name": " "}
    elif structure_name == "leaders":   
        try:     
            initials, name, date, score = struct.unpack("<3s16s10sI", data)
            return {"initials": initials.decode().strip('\0'), "full_name": name.decode().strip('\0'),  "date": date.decode().strip('\0') , "score": score}
        except:
            return None
    elif structure_name == "tournament":
        try:
            initials, score, game, index = struct.unpack("<3sIBB", data)
            return {"initials": initials.decode().strip('\0'), "score": score, "game": game, "index": index}
        except:
            return None
    elif structure_name == "individual":
        try:
