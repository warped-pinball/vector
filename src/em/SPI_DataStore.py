...
Customize this for EM - needs game data store class
...
def show_mem_map():
    ...
    # ensure EMData start is computed relative to extras so show_mem_map reflects current sizes
    memory_map["EMData"]["start"] = memory_map["extras"]["start"] - (memory_map["EMData"]["size"] * memory_map["EMData"]["count"])
...
def serialize(record, structure_name):
    if structure_name == "names":
        ...
        if len(packed) < record_size:
            packed = packed + (b"\0" * (record_size - len(packed)))
        return packed
    else:
