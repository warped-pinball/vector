...
def _get_machine_score(player):
    ...
    try:
        if S.gdata.get("InPlay", {}).get("Type", 0) != 0:
            ...
            score_bytes = 0 #shadowRam[start : start + S.gdata["InPlay"]["ScoreBytes"]]
...
def _get_ball_in_play():
    ...
    try:
        ...
        if ball_in_play["Type"] == 1:
            ret_value = 0 #shadowRam[ball_in_play["Address"]]           
            if 0: #shadowRam[S.gdata["InPlay"]["GameActiveAdr"]] != S.gdata["InPlay"]["GameActiveValue"]:
                ret_value = 0   
