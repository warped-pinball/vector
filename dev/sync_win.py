...
def autodetect_pico_port():
    """Auto-detect the Pico port using mpremote and filter out unwanted ports."""
    print("Auto-detecting Pico port...")

    try:
        # Attempt to detect the Pico port using `mpremote`
        result = subprocess.run("mpremote connect list", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
            output = result.stdout.decode().strip()
            if output and "COM1" not in output:
                port = output.split('\n')[0].split()[0]
                print(f"Port found with mpremote: {port}")
                return port

        # Fallback method: scan available COM ports, filtering out unwanted ports
        ports = serial.tools.list_ports.comports()
        print("Available ports:", ports)

        # Exclude COM1 and Bluetooth ports, keeping only the highest-numbered valid port
        valid_ports = [
            port.device for port in ports
            if port.device != "COM1" and "Bluetooth" not in port.description
        ]

        print("Valid ports (filtered):", valid_ports)

        if valid_ports:
            # Select the highest-numbered valid port
            selected_port = sorted(valid_ports, key=lambda x: int(x[3:]))[-1]
            print(f"Selected port: {selected_port}")
            return selected_port

        print("Unable to detect Pico port. Please specify the correct port manually.")
        
    except Exception as e:
        print(f"Error detecting Pico port: {e}")

    # Exit if no port found
    sys.exit(1)
...
def wipe_pico():
    ...
    # Define the script to delete all files
    pico_script = """
import os
def remove(path):
    print(path)
...
"""
    ...
    # Build the command with `mpremote` to execute the above script on the Pico
    cmd = f"mpremote connect {PICO_PORT} exec \"{pico_script}\""
    
    # Run the command
    result = subprocess.run(cmd, shell=True)
    print("Wipe filesystem result =", result)
