...
def main():
    ...
    def print_small_files(files_list):
        ...
        size_width = max(
            len(headers_small_files[0]),
            max((len(f["size_str"]) for f in files_list), default=0),
        )
        loss_width = max(
            len(headers_small_files[1]),
            max((len(f["wasted_space_str"]) for f in files_list), default=0),
        )
        percent_width = max(
            len(headers_small_files[2]),
    ...
    def print_inefficient_files(files_list):
        # Calculate column widths
        size_width = max(
            len(headers_inefficient_files[0]),
            max((len(f["size_str"]) for f in files_list), default=0),
        )
        over_block_width = max(
            len(headers_inefficient_files[1]),
            max((len(f["bytes_over_block_str"]) for f in files_list), default=0),
        )
        loss_width = max(
            len(headers_inefficient_files[2]),
            max((len(f["wasted_space_str"]) for f in files_list), default=0),
        )
        percent_width = max(
            len(headers_inefficient_files[3]),
            max((len(f"{f['wasted_percentage']:.2f}%") for f in files_list), default=0),
        )
